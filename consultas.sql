...
BEGIN
	IF _id = 0 THEN
    	INSERT INTO noticias (autor,autor_correo, titulo, contenido, extracto, fecha, hora, estado, status, img_principal, img_autor)
        VALUES (_autor, _autor_correo, _titulo, _contenido, _extracto, _fecha, _hora, _estado, _status, _img_principal, _img_autor);
...
BEGIN
	IF _nivel = 3 THEN
    	INSERT INTO voluntarios (correo, nombres, apellidos, fechareg)
        VALUES (_correo, _nombres, _apellidos, _fechareg);
    ELSE
    	INSERT INTO gestores (correo, nombres, apellidos, fechareg)
        VALUES (_correo, _nombres, _apellidos, _fechareg);
    END IF;
...
//   console.log(`${username} se ha unido al grupo`);
